...
<div class="card shadow mt-3 mx-5">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Edit Jadwal Latihan</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('data-jadwal-latihan.update', $jadwalLatihan->id) }}" method="post">
            @csrf
            @method('put')
            ...
            <div class="form-group">
                <div class="mb-3">
                    ...
                    <select class="custom-select @error('pelatih_id') is-invalid @enderror" name="pelatih_id" id="pelatih_id" required>
                        ...
                        @foreach ($pelatih as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $jadwalLatihan->pelatih_id ? 'selected' : '' }}>{{ $item->nama }}</option>
                        @endforeach
                    </select>
                    @error('pelatih_id')
                ...
                <div class="mb-3">
                    ...
                    <input type="time" class="form-control @error('jam') is-invalid @enderror" id="jam" name="jam" value="{{ old('jam', $jadwalLatihan->jam) }}" required>
                    @error('jam')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('lokasi') is-invalid @enderror" id="lokasi" name="lokasi" value="{{ old('lokasi', $jadwalLatihan->lokasi) }}" placeholder="Lokasi" required>
                    @error('lokasi')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
